...
#if defined(NDEBUG)
# error "Wesleycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3fe6e2c83b0eacd8730040dd7376fc37b64724a0053ff321d1d20c04edf67f41");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wesleycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wesleycoin Signed Message:\n";
...
{
    // Wesleycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000000); // Wesleycoin: 10 million blocks in ~17.9 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Wesleycoin: 1.5 days
static const int64 nTargetSpacing = 1 * 60; // Wesleycoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wesleycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wesleycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2b1e141f01d2bcebdb58e9242065f24e02cfeb5d98d7ef4d06b876bf3cc23d36");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Washington Post 12/Jun/2018 The Quest of Laurene Powell Jobs";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d05f670a5587235bff2ec40149bb32bc51b600ebdbf8c205aa8aaf1ff6ca379ed1490cd7b432bbea801454c509afb4b3fa20d68156e4a93835d4b117e7a80efb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528850540;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085630938;

        if (fTestNet)
        {
            block.nTime    = 1528850507;
            block.nNonce   = 386034089;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8a9fa0d29b0da4a4439b84f4f08694f87a4aa5edfd7724144eb7853227fed1bd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xcc, 0xb6, 0xd7 }; // Wesleycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WesleycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WesleycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WesleycoinMiner(CWallet *pwallet)
{
    printf("WesleycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wesleycoin-miner");
    ...
    try { loop {
        ...
        printf("Running WesleycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WesleycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WesleycoinMiner, pwallet));
}
